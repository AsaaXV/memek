let handler  = async (m, { conn, text:sksk }) => {
  ...
  mention = sksk.includes('--taguser') ? metadata : []
  ...
  } else conn.copyNForward(id, conn.send2ButtonLoc, await (await fetch(nani)).buffer(), '', 'LIST ALL MENU', '.menu', 'OWNER', '.owner', content, m)
  }
